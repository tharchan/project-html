...
public class EmployeeList {
	...
	protected void Database_Save(){
		try {
		    ...
		    for(Entry<String, Employee> data : employeeList.entrySet()) {
		        String key = data.getKey();		        
		        outFile.println(employeeList.get(key).getSSNumber()+","+employeeList.get(key).getFirstName()+","+employeeList.get(key).getLastName()+","+employeeList.get(key).getDateOfBirth()+","+employeeList.get(key).getAddress()+","+employeeList.get(key).getPhone()+","+employeeList.get(key).getEmail()+","+employeeList.get(key).getJobTitle());                     
		    }
		    outFile.close();
		} catch (IOException e) {
			// TODO Auto-generated catch block
